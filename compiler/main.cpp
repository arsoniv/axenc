...
int main(int argc, char **argv) {
  ...
  if (!headerOutputFile.empty()) {
    // emit header file
    std::ofstream headerOut(headerOutputFile);
    if (!headerOut) {
      fprintf(stderr, "Could not open header output file: '%s'\n", headerOutputFile.c_str());
      return EXIT_FAILURE;
    }

    headerOut << "// this is a header auto generated by the axenc compiler\n\n";

    for (const auto &_class : *parser->getClasses()) {
      _class->emitHeader(headerOut);
    }

    for (const auto &func : *parser->getFunctions()) {
      if (func->isExported()) {
        func->emitHeader(headerOut);
      }
    }
